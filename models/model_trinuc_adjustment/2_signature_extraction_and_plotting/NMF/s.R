...
SBS84_exposures = read_tsv("/g/strcombio/fsupek_cancer3/malvarez/WGS_tumors/somatic_variation/TCGA_PCAWG_Hartwig_CPTAC_POG_MMRFCOMMPASS/AID_SHM/3_extract_SBS84__sort_AID_vs_nonAID_samples/lymphoid_samples_SBS84_exposure_K3.tsv") %>% 
...
for(optimal_k in seq(2, maxK)){
  ...
  exposures = nmf_res$h %>% 
    ...
    merge(metadata, all = T) %>% 
    filter(!is.na(Exposure)) %>% 
    separate(sample_id, into = c("sample_id", "mut x-fold"), sep = "__", fill = "right") %>% 
    # simulated pos controls dont have SBS84, so add them an average fake one so that their size is not tiny in the plots (as point size is dependent on SBS84 raw exposure)
  ...
  if(length(signatures_lower_exposure_than_limit) > 0){
    filtered_exposures = filtered_exposures %>% 
      bind_rows(data.frame(signatures_lower_exposure_than_limit) %>% 
                  `colnames<-`("Signature") %>% 
                  mutate(dataset = filtered_exposures$dataset[1]))}
  ...
  exposures_plot = ggplot(filtered_exposures, 
    ...
    scale_shape_manual(values = c(21, 23, 25, 1, 2)) +
    guides(fill = guide_legend(override.aes = list(size=4, shape=21)),
           shape = guide_legend(override.aes = list(size=4))) +
    ggrepel::geom_text_repel(aes(label = sample_id), #paste(sample_id, info1)),
                             size = 3,
                             force = 5,
                             position = pos,
