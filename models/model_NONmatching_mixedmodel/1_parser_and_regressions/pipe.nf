...
median_scores.into{median_scores_for_binarize_scores; median_scores_for_load_sample_somatic_muts_overlap_feature_maps_run_regressions; median_scores_for_sim_pos_con}
...
process load_feature_maps {
    ...
    then
        ...
        then
            # there is a conda environment named "R"
            conda activate R
            Rscript $PWD/bin/2_load_feature_maps.R ${dnarep_marks} ${chromatin_features} ${chromosome}
        else
            # no conda environment named "R"
            Rscript $PWD/bin/2_load_feature_maps.R ${dnarep_marks} ${chromatin_features} ${chromosome}
        fi
...
// the same map_features is passed to 2 processes
map_features.into{map_features_to_collect; map_features_for_binarize_scores}

// rowbind all chromosomes' raw score map features, will be read by 5th process's R script (hardcoded)
map_features_to_collect
...
process load_sample_somatic_muts_overlap_feature_maps_run_regressions {
    ...
    then
        ...
        then
            # there is a conda environment named "R"
            conda activate R
            Rscript $PWD/bin/5_load_sample_somatic_muts_overlap_feature_maps_run_regressions.R ${sample} ${somatic_data} ${metadata} ${dnarep_marks} ${chromatin_features} ${offset} $PWD ${low_mappability_regions} ${median_scores_collected}
        else
            # no conda environment named "R"
            Rscript $PWD/bin/5_load_sample_somatic_muts_overlap_feature_maps_run_regressions.R ${sample} ${somatic_data} ${metadata} ${dnarep_marks} ${chromatin_features} ${offset} $PWD ${low_mappability_regions} ${median_scores_collected}
        fi
...
process sim_pos_con {
    ...
    time = 6.hour
    memory = { (params.memory_process6 + 5*(task.attempt-1)).GB }
    ...
    then
        ...
        then
            # there is a conda environment named "R"
            conda activate R
            Rscript $PWD/bin/6_simulate_pos_controls.R ${somatic_data} ${metadata} ${dnarep_marks} ${chromatin_features} ${offset} $PWD ${low_mappability_regions} ${mutation_foldinc} ${median_scores_collected}
        else
            # no conda environment named "R"
            Rscript $PWD/bin/6_simulate_pos_controls.R ${somatic_data} ${metadata} ${dnarep_marks} ${chromatin_features} ${offset} $PWD ${low_mappability_regions} ${mutation_foldinc} ${median_scores_collected}
        fi
        ...
        # no conda
        Rscript $PWD/bin/6_simulate_pos_controls.R ${somatic_data} ${metadata} ${dnarep_marks} ${chromatin_features} ${offset} $PWD ${low_mappability_regions} ${mutation_foldinc} ${dnarep_marks_simulate} ${median_scores_collected}
    fi
    """
}
