...
reg_coeffs_repdefsig = read_tsv("../../../1_parser_and_regressions/res/results.tsv") %>% 
  dplyr::select(c("sample_id", contains("AID"))) %>% 
...
MW_plot_log10_no_2outliers = ggplot(SBS84exposures_regcoeffs %>% 
  ...
  xlab("Model prediction\n#SNVs ~ 6×DNArepmarks + WG-vs-AIDtargets + (1|RepliSeq) + (1|3NContext) + log(sum(RepliSeq×3NContext))") +
  ylab("SBS84 raw exposure (log10 scale)") +
  theme(text = element_text(size = 25),
        axis.title.x = element_text(size = 20),
        legend.title = element_blank())
ggsave("MW_plot_SBS84_raw_exposures_log10_no1outlier.jpg",
